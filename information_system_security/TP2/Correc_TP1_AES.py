...
# If you're using this for TP2, here's what you need to know : you only need
# the functions called "AES_Box" and "AES_Inv_Box". Each has two arguments :
# first argument is the block (16 bytes) to encrypt or decrypt, and second
# argument is the key. Both argument are strings in this case (with 16 bytes
# = 16 chars for the message, and 16/24/32 bytes for the key). You can just
# import this file and call AES_Box(block, key) to obtain the encrypted block,
# and similarly AES_Inv_Box(block, key) to obtain the decrypted block.
...
def AES_Box(message, key):
    ...
    for i in range(1, steps + 1):
        ...
        if i < steps:
            message = MixColumn(message, MixColMatrix)
...
def AES_Inv_Box(message, key):
    ...
    for i in range(steps, 0, -1):
        ...
        message = AddRoundKey(message, next_key)
        if i < steps:
            message = MixColumn(message, MixColInverse)
...
def AES_ECB_Encrypt(message, key):
    ...
    Length_Padded = L + K
    byte = chr(K)
    padded_message = message + K * byte

    # Slice padded message into blocks
    Blocks = []
    Number_Blocks = Length_Padded // 16
    for i in range(Number_Blocks):
        Blocks.append(padded_message[16 * i:16 * (i + 1)])

    # Encrypt each block
    Encrypted_Blocks = []
    for i in range(Number_Blocks):
        Encrypted_Blocks.append(AES_Box(Blocks[i], key))

    # Build the ciphertext
    Ciphertext = ''.join(item for item in Encrypted_Blocks)
    return Ciphertext


def AES_ECB_Decrypt(cipher, key):
    # Slicing the cipher in blocks
    L = len(cipher)
    Crypted_Blocks = []
    Number_Blocks = L // 16
    for i in range(Number_Blocks):
        Crypted_Blocks.append(cipher[16 * i:16 * (i + 1)])

    # Decrypting each block
    Decrypted_Blocks = []
    for i in range(Number_Blocks):
        Decrypted_Blocks.append(AES_Inv_Box(Crypted_Blocks[i], key))

    # reconstructing the padded message, and removing the padding

    pad_message = ''.join(item for item in Decrypted_Blocks)
    K = ord(pad_message[-1])
    if K < 1 or K > 16:
        raise ValueError("decryption does not have a valid padding")
    message = pad_message[:L - K]

    return message

# 9message = "Can you smell what the Rock is cooking?"
# key = "You can't see me"
# key2 = "The greatest of all time"
# key3 = "Sixteen time wwe world champion!"

# ciphertext = AES_ECB_Encrypt(message,key)
# cipher2 = AES_ECB_Encrypt(message, key2)
# cipher3 = AES_ECB_Encrypt(message, key3)

# plaintext = AES_ECB_Decrypt(ciphertext,key)
# plain2 = AES_ECB_Decrypt(cipher2, key2)
# plain3 = AES_ECB_Decrypt(cipher3, key3)
# print(message)
# print(ciphertext)
# print(cipher2)
# print(cipher3)
